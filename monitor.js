...
const CASA_RADAR03 = process.env.CASA_RADAR03 || 'ALEAL';
...
async function enviarEmail(novas) {
  ...
  const html = `
    ...
    <div style="font-family:Arial,sans-serif;max-width:900px;margin:0 auto">
      <h2 style="color:#1a3a5c;border-bottom:2px solid #1a3a5c;padding-bottom:8px">
        🏛️ Assembleia Legislativa de Alagoas — ${novas.length} nova(s) proposição(ões)
      </h2>
  ...
  await transporter.sendMail({
    from: `"Monitor Alagoas" <${EMAIL_REMETENTE}>`,
    to: EMAIL_DESTINO,
    subject: assuntoEmailClienteCitado(novas, `🏛️ Alagoas: ${novas.length} nova(s) proposição(ões) — ${new Date().toLocaleDateString('pt-BR')}`),
    html,
  });
